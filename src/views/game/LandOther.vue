  ...
  <div class="cont-land" @click="clickHandler()">
    ...
    <div class="cont-ani">
      ...
      <div class="cont-glove" ref="glove" v-show="showGlove">
        ...
        <span class="tip">{{ toFixed2(amount) }}</span>
      </div>
      <div class="cont-option" v-if="showOption && (!getCanLand())">
        ...
        <div class="option-list1" v-if="optionType == 4">
      ...
      <div class="cont-option" v-if="showOption && getCanLand()">
        ...
        <div class="option-list5" v-if="optionType == 4">
          <span class="menu" @click="clickMenuHandler('plant')">{{ $t('plant') }}</span>
          ...
          <span class="menu" @click="clickMenuHandler('rent')">{{ $t('rent') }}</span>
        </div>
...
import { formatTime, formatTime3, maskString3, toFixed2 } from '@/utils/util';
...
function clickHandler() {
  ...
  if (getCanLand()) {
    if (getCanPlayShovel()) {
      ...
      sendReceiveAction()
    } else if ((!info.isPlantOwner) && (info.isLandOwner) && (info.myGrowStatus == 2 || info.myGrowStatus == 3)) {
      sendUseShovelAction()
    ...
    if (info.isPlantOwner) {
      if (getCanPlayShovel()) {
        ...
        showOptionAction(4)
      } else if (info.myPlantStatus == 2) {
        sendUsePesticideAction()
      } else if (info.myPlantStatus == 3) {
        sendUseWaterAction()
      } else if (info.myGrowStatus == 2) {
        sendReceiveAction()
      } else if ((!info.isPlantOwner) && (info.myGrowStatus == 3)) {
        sendUseShovelAction()
      } else if (info.hasPlant && info.myGrowStatus == 1) {
        showOptionAction(1)
      }
    } else {
      if ((!info.isPlantOwner) && (info.myGrowStatus == 2)) {
        sendUseGloveAction()
      } else if ((!info.hasPlant) && (info.myLandStatus == 3)) {
        showOptionAction(4)
      } else if (getCanPlayAdd() && info.hasPlant && info.myGrowStatus == 1) {
        showOptionAction(1)
      }
...
function calcMyInfo(lastInfo) {
  ...
  info['landUserAddress'] = info.landUserAddress
  info['isShowAddr'] = info.landUserAddress && (info.landUserAddress != props.currAddr)
